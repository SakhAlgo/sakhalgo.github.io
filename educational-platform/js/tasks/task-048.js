export default class Task048Validator {
    async validate(html, css, js, samples, config) {
        ...
        checks.push({ label: 'Функция canAccess объявлена', passed: hasFunction, hint: 'Создайте функцию canAccess(age, hasLicense)' });
        if (hasFunction) score += 15;

        // Проверка использования if с age >= 18
        const hasAgeCheck = /if\s*\(\s*age\s*>=\s*18/.test(js);
        checks.push({ label: 'Проверка age >= 18', passed: hasAgeCheck, hint: 'Добавьте условие if (age >= 18)' });
        if (hasAgeCheck) score += 15;

        // Проверка вложенного if (hasLicense)
        const hasNestedIf = /if\s*\(\s*hasLicense\s*\)/.test(js) || /if\s*\(\s*hasLicense\s*===?\s*true\s*\)/.test(js);
        checks.push({ label: 'Проверка hasLicense внутри if', passed: hasNestedIf, hint: 'Добавьте вложенный if (hasLicense)' });
        ...
        checks.push({ label: 'Возвращает "drive" при age >= 18 и hasLicense === true', passed: returnsDrive, hint: 'Добавьте return "drive"' });
        if (returnsDrive) score += 15;

        // Проверка возврата 'no license'
        const returnsNoLicense = /return\s+['"]no\s*license['"]/.test(js);
        checks.push({ label: 'Возвращает "no license" при age >= 18 и hasLicense === false', passed: returnsNoLicense, hint: 'Добавьте return "no license"' });
        if (returnsNoLicense) score += 15;

        // Проверка возврата 'too young'
        const returnsTooYoung = /return\s+['"]too\s*young['"]/.test(js);
        checks.push({ label: 'Возвращает "too young" при age < 18', passed: returnsTooYoung, hint: 'Добавьте return "too young"' });
