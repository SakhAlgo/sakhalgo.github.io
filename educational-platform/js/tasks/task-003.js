export default class Task003Validator {
    async validate(html, css, js, samples, config) {
        ...
        checks.push({ label: 'Тег <ul> присутствует', passed: hasUl, hint: 'Добавьте <ul>' });
        if (hasUl) score += 20;
        const liCount = (html.match(/<li[^>]*>/gi) || []).length;
        const hasThreeLi = liCount >= 3;
        checks.push({ label: 'Три элемента <li>', passed: hasThreeLi, hint: 'Добавьте 3 элемента <li>' });
        ...
        checks.push({ label: 'Элементы: Молоко, Хлеб, Яйца', passed: allItems, hint: 'Проверьте названия продуктов' });
