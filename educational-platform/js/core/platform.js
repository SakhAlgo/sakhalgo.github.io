...
export class EducationalPlatform {
    ...
    async init() {
        ...
        // Восстановить последнее задание
        const lastId = localStorage.getItem('educode_last_task');
        if (lastId) await this.loadTask(lastId);
    }
    ...
    async _loadSamples(taskPath, config) {
        const [html, css, js] = await Promise.all([
            fetch(`${taskPath}${config.sampleFiles?.html || 'sample.html'}`).then(r => r.text()).catch(() => ''),
            fetch(`${taskPath}${config.sampleFiles?.css  || 'sample.css'}`).then(r => r.text()).catch(() => ''),
            fetch(`${taskPath}${config.sampleFiles?.js   || 'sample.js'}`).then(r => r.text()).catch(() => ''),
        ]);
        return { html, css, js };
    }
    ...
    async checkSolution() {
        ...
        try {
            ...
            if (result.passed) {
                StorageManager.markCompleted(this.currentTask.id, result.score, { html, css, js });
                this.progress = StorageManager.load();
                this.updateGlobalProgress();
                this.renderTaskList();
                this._markTaskCompleted(this.currentTask.id);
                this.toast(`🎉 Отлично! Задание выполнено! Счёт: ${result.score}%`, 'success');
            } else {
                this.toast(`Ещё не так! Счёт: ${result.score}% (нужно ${this.currentTask.passThreshold}%)`, 'error');
            }
    ...
    _updateTaskHeader(taskInfo, config) {
        ...
        if (tags)  tags.innerHTML = (taskInfo.tags || []).map(t => `<span class="tag">${t}</span>`).join('');

        if (reqs) {
            reqs.innerHTML = (config.requirements || []).map(r =>
    ...
    _showCheckResults(result) {
        ...
        const scoreColor = result.passed ? 'var(--success)' : result.score >= 50 ? 'var(--warning)' : 'var(--danger)';
            ...
            <div class="result-score">
                ...
                <div class="score-details">
                    ...
                    <p>${result.passed
                        ...
                        : `Нужно набрать ${this.currentTask.passThreshold}%. Проверьте подсказки ниже.`}</p>
                </div>
            </div>
            ${result.checks.map(c => `
    ...
    bindUI() {
        ...
        // Editor actions
        document.getElementById('formatBtn')?.addEventListener('click', () => this.editor.formatActive());
        document.getElementById('clearBtn')?.addEventListener('click', () => {
            if (confirm('Очистить текущую вкладку?')) {
                const ed = this.editor.editors[this.editor.activeTab];
                if (ed) { ed.value = ''; this.editor.updateLineNumbers(); }
            }
        });
        document.getElementById('resetCodeBtn')?.addEventListener('click', () => {
            if (this.currentTask && confirm('Сбросить код к шаблону?')) {
                const starter = this._getStarterCode(this.currentTask.id, this.currentTask.config);
        ...
        // Fullscreen
        document.getElementById('fullscreenSample')?.addEventListener('click', () => {
            this.preview.openFullscreen(document.getElementById('sampleFrame'), '🎯 Эталон');
        });
        document.getElementById('fullscreenUser')?.addEventListener('click', () => {
            this.preview.openFullscreen(document.getElementById('userFrame'), '🖥️ Ваш результат');
        });
        ...
        document.getElementById('sidebarToggle')?.addEventListener('click', () => {
            ...
            if (ws) ws.style.left = document.getElementById('sidebar').classList.contains('collapsed') ? '52px' : '';
        });
    ...
    showConfirm(title, text, onConfirm) {
        ...
        const cleanup = () => {
            document.getElementById('modalConfirm').removeEventListener('click', confirm);
