export default class Task046Validator {
    async validate(html, css, js, samples, config) {
        ...
        checks.push({ label: 'Функция checkNumber объявлена', passed: hasFunction, hint: 'Создайте функцию checkNumber(num)' });
        ...
        const hasElseIf = /\}\s*else\s+if\s*\(/.test(js) || /\belse\s+if\b/.test(js);
        ...
        checks.push({ label: 'Возвращает "positive" для чисел > 0', passed: returnsPositive, hint: 'Добавьте return "positive"' });
        if (returnsPositive) score += 15;

        // Проверка возврата 'negative'
        const returnsNegative = /return\s+['"]negative['"]/.test(js);
        checks.push({ label: 'Возвращает "negative" для чисел < 0', passed: returnsNegative, hint: 'Добавьте return "negative"' });
        if (returnsNegative) score += 10;

        // Проверка возврата 'zero'
        const returnsZero = /return\s+['"]zero['"]/.test(js);
        checks.push({ label: 'Возвращает "zero" для числа 0', passed: returnsZero, hint: 'Добавьте return "zero"' });
