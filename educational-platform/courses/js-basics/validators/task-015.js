export default class Task015Validator {
  async validate(html, css, js, samples, config) {
    ...
    checks.push({
      label: "lastName = Doe",
      passed: hasLast,
      hint: "Создайте lastName = Doe",
    });
    if (hasLast) score += 20;
    const hasFull =
      /fullName\s*=\s*firstName\s*\+\s*['\"\s]+\s*\+\s*lastName/.test(js);
    checks.push({
      label: "Конкатенация в fullName",
      passed: hasFull,
      hint: "Создайте fullName через конкатенацию",
    });
    if (hasFull) score += 30;
