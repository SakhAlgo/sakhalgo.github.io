    ...
    const difficultyLabel =
      task.difficulty === 'easy'
        ...
        : task.difficulty === 'medium'
          ? 'Среднее'
          : 'Тяжелое';
    tasksHtml += `<div class="task-card ${isCompleted ? 'completed' : ''}" data-task-idx="${idx}"><div class="task-header" onclick="toggleTaskBody(this, ${idx})"><div class="task-number">${isCompleted ? '✓' : idx + 1}</div><div class="task-info"><h4>${escapeHtml(task.title)}</h4><p>${escapeHtml(task.desc)}</p></div><div class="difficulty ${task.difficulty}">${difficultyLabel}</div></div>`;
  });
  main.innerHTML = `<div class="card"><h2>${escapeHtml(theme.theory.title)}</h2><div class="subtitle">${escapeHtml(theme.theory.subtitle)}</div>${theme.theory.content}</div><h3 style="margin-bottom:20px">📋 Практические задания (${theme.tasks.length} заданий) — код с правильными отступами</h3><div class="tasks-section">${tasksHtml}</div><div class="nav-buttons"><button class="btn" ${currentTheme === 0 ? 'disabled' : ''} onclick="prevTheme()">← Предыдущая тема</button><button class="btn btn-primary" onclick="nextTheme()">Следующая тема →</button></div>`;
}
